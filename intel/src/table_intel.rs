...
impl<'a> TableIntel<'a>{
    ...
    fn refers_to(&self, arg_table: &Table) -> bool {
        ...
        for fk in this_foreign_keys{
            ...
            if *foreign_table == arg_table.name 
                && same_column_content {
                    return true;
            }
        }
    ...
    pub fn get_one_one_tables<'t>(&self, tables: &'t Vec<Table>)-> Vec<&'t Table> {
        ...
        for table in tables{
            ...
            if table_intel.refers_to(self.0)
...
mod test{
    ...
    fn users_table(){
        ...
        let one_one_tables = table_intel.get_one_one_tables(&all_tables);
        assert_eq!(one_one_tables.len(), 1);
        assert_eq!(one_one_tables[0].name, TableName::from("bazaar.user_location"));
        ...
        for table in &all_tables{
            ...
            if !indirect_tables.is_empty(){
                for indirect in indirect_tables{
                    println!("          indirect --> ({}) --> ({})", indirect.linker.name.name, indirect.indirect_table.name.name); 
                }
            }
        }
        panic!();
