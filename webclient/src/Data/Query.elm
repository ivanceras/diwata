...
orderParser arg =
    if String.isEmpty arg then
        ...
        let splinters = String.split "." arg
            reverse = List.reverse splinters
            column = List.drop 1 reverse  
                        |> List.reverse
                        |> String.join "."
            last = List.head reverse
        in
            case last of
                Just "asc" -> 
                    Just { column = column
                ...
                _ -> 
                    Just { column = (String.join "." splinters)
                         , direction = ASC
                         }
...
maybeFilterParser =
    Url.custom "MAYBE_FILTER" <| \segment -> 
